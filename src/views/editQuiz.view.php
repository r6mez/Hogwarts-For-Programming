...
<body>
    ...
    <div class="content-wrapper">
        <h1>Edit Quiz</h1>
        <form action="/manageQuizzes/edit" method="POST">
            <input type="hidden" name="id" value="<?= $quiz['id'] ?>">
            <label for="id_cour">Course:</label>
            <select id="id_cour" name="id_cour" required>
                <?php foreach ($courses as $course): ?>
                    <option value="<?= $course['id'] ?>" <?= $course['id'] == $quiz['id_cour'] ? 'selected' : '' ?>>
                        <?= htmlspecialchars($course['name']) ?>
                    </option>
                <?php endforeach; ?>
            </select>
            <label for="score">Score:</label>
            <input type="number" id="score" name="score" value="<?= htmlspecialchars($quiz['score']) ?>" required>
                ...
                <?php foreach ($questions as $question): ?>
                    <tr>
                        ...
                        <td>
                            ...
                            <a href="/manageQuestions/delete?id=<?= $question['id'] ?>&quiz_id=<?= htmlspecialchars($quiz['id']) ?>" onclick="return confirm('Are you sure?')">Delete</a>
                        </td>
                    </tr>
                <?php endforeach; ?>
            </tbody>
